...
PartyTab:CreateToggle({
    Name = "Color Cheese: Auto Win",
    CurrentValue = false,
    Callback = function(enabled)
        if enabled then
            -- Conexão para monitorar a cor
            getgenv().colorCheeseConnection = game:GetService("RunService").Heartbeat:Connect(function()
                local targetColor = game:GetService("ReplicatedStorage").Game["Color Cheese"].ColorSelect.Value
                local player = game.Players.LocalPlayer
                
                -- Procurar a part correspondente
                local colorParts = workspace.Map["Color Cheese"].colors:GetChildren()
                local correctPart = nil
                
                for _, part in ipairs(colorParts) do
                    if part:IsA("Part") and part.BrickColor == targetColor then
                        correctPart = part
                        break
                    end
                end
                ...
                if correctPart and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    local teleportCFrame = CFrame.new(correctPart.Position + Vector3.new(0, 3, 0))
            ...
            Rayfield:Notify({
                ...
                Content = "Teleportando para a cor correta automaticamente!",
                Duration = 3,
                Image = 4483362458
            })
        else
            if getgenv().colorCheeseConnection then
                getgenv().colorCheeseConnection:Disconnect()
                Rayfield:Notify({
                    Title = "Auto Win Desativado",
                    Content = "Modo color cheese desligado",
                    Duration = 2,
                    Image = 4483362458
                })
            end
